...
class NDCGLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        ndcgs = torch.tensor([], device="cuda:0")
        for i in range(len(outputs)):
            output = outputs[i]
            target = targets[i]
            pre_top3 = torch.topk(output, 3).indices
            amounts = torch.topk(output, 3).values
            pre_top3_amounts = target[pre_top3]
            act_top3_amounts = torch.topk(target, 3).values
            dcg = torch.tensor(0., requires_grad=True)
            idcg = torch.tensor(0., requires_grad=True)
            for i, (pre_amount, act_amount) in enumerate(zip(pre_top3_amounts, act_top3_amounts)):
                dcg = torch.add(dcg, torch.div(pre_amount, torch.log2(torch.tensor(2 + i))))
                idcg = torch.add(idcg, torch.div(act_amount, torch.log2(torch.tensor(2 + i))))
            ndcg = torch.div(dcg, idcg) if idcg != 0 else torch.tensor(0., requires_grad=True)
            ndcg = torch.clip(ndcg, max=1).to("cuda:0")
            ndcgs = torch.cat((ndcgs, ndcg.unsqueeze(dim=0)), 0)
        return torch.mean(ndcgs)
...
class LambdaLoss(nn.Module):
    """
    src: https://github.com/allegro/allRank
    """
    def __init__(self):
        # --------------------------------------------
        # Initialization
    ...
    def forward(self, y_pred, y_true, eps=DEFAULT_EPS, padded_value_indicator=PADDED_Y_VALUE, weighing_scheme=None, k=None, sigma=1., mu=10.,
